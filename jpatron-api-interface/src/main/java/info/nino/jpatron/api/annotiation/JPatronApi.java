package info.nino.jpatron.api.annotiation;
...
/**
 * Annotation for tagging JPatron API endpoints
 * Binds to JPatronApiRequestFilter which will automatically deserialize HTTP query-parameters to JPatronApiRequest object
 *
 */
@Inherited
@NameBinding
...
public @interface JPatronApi
{
    ...
    /**
     * List of field-paths used for search targets in request - represented as exact or wildcard values
     * By default only root String fields of the result list object are used ('.')
     * '.' - matches all String fields in current path
     ...
     * e.g. 'person.name' - path matches exact field 'name' under the 'person' object
     * e.g. 'person.' - path matches all String fields directly in the 'person' object (but none of the fields in the nested objects)
     * @return array of search field-paths
     */
     ...
     * 'object.field' - exact path/field matching where 'object' is nested-object in the root of result-object and 'field' is exact property name in that object
     * e.g. 'title' - path matches exact field 'title' in root object
     * e.g. 'person.name' - path matches exact field 'name' under the 'person' object
