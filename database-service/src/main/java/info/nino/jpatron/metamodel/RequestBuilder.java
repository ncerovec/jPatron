...
public class RequestBuilder<T>
{
    ...
    public static <T> RequestBuilder<T> init(ApiRequest<T> apiRequest)
    {
        ...
        MapUtils.emptyIfNull(apiRequest.getQueryParams().getSort()).forEach((fieldPath, value) ->
        {
            PageRequest.Sort.Direction direction = PageRequest.Sort.Direction.resolveDirectionSign(value.getValue());
            requestBuilder.addSorting(fieldPath, direction);
        });
        ...
        MapUtils.emptyIfNull(apiRequest.getQueryParams().getFilters()).forEach((clazz, filterPaths) ->
        {
            ...
            {
                filterValues.asMap().forEach((operator, values) ->
                {
                    QueryExpression.CompareOperator compareOperator = QueryExpression.CompareOperator.valueOf(operator);
        ...
        MapUtils.emptyIfNull(apiRequest.getQueryParams().getSearches()).forEach((clazz, searchFields) ->
        {
            ...
            {
                searchValues.asMap().forEach((modifier, values) ->
                {
                    QueryExpression.CompareOperator compareOperator = QueryExpression.CompareOperator.LIKE;
                    QueryExpression.ValueModifier valueModifier = QueryExpression.ValueModifier.valueOf(modifier);
        ...
        MapUtils.emptyIfNull(apiRequest.getQueryParams().getMetaValues()).forEach((clazz, metaFields) ->
        {
            metaFields.forEach((valueField, labelValues) ->
            {
                ...
                {
                    QueryExpression.Func func = QueryExpression.Func.valueOf(function);
                    labelFields.forEach(labelField ->
                    {
                        if(StringUtils.isNotBlank(labelField))
                        {
                            requestBuilder.addMeta(labelField, func, valueField);
                        }
                        else
                        {
                            requestBuilder.addMeta(func, valueField);
                        }
                    });
                });
            });
    ...
    public RequestBuilder<T> addSorting(String sortFieldPath, PageRequest.Sort.Direction direction)
    {
    ...
    public RequestBuilder<T> addMeta(QueryExpression.Func function, String valueFieldPath)
    {
    ...
    public RequestBuilder<T> addMeta(String labelFieldPath, QueryExpression.Func function, String valueFieldPath)
    {
